...
#annualized return per stock in portfolio

annualized <- winners %>%
  select(-StockSymbol, -Industry, -SubIndustry, -Cluster, -CV) %>%
  rowwise() %>%
  mutate(
    n_months = sum(!is.na(c_across(everything()))),
    gross = prod(1 + c_across(everything()), na.rm = TRUE),
    annualized_return = gross^(12 / n_months) - 1
  ) %>%
  ungroup() %>%
  select(annualized_return)

tableau <- cbind(winners, annualized)

#time series data

time_series_nasdaq <- nasdaq_with_clusters %>%
  select(-StockSymbol, -Industry, -SubIndustry, -Cluster) %>%
  mutate(across(everything(), ~ .x+1)) %>%
  summarise(across(where(is.numeric), ~ prod(.x)^(1/length(.x))))

time_series_longer_nasdaq <- pivot_longer(time_series_nasdaq, cols = c(1:120), values_to = "NASDAQ_return", names_to = "time") %>%
  mutate(time = str_extract(time, "\\d{4}\\.\\d{2}"),
         time = ym(time),
         month = month(time),
         year = year(time)) %>%
  mutate(
    wealth_NASDAQ = 100 * cumprod(coalesce(NASDAQ_return, 1))    # $100 growing month by month
  )
  


time_series_winner <- winners %>%
  ungroup() %>%
  select(-StockSymbol, -Industry, -SubIndustry, -Cluster, -CV) %>%
  mutate(across(everything(), ~ .x+1)) %>%
  summarise(across(where(is.numeric), ~ prod(.x)^(1/length(.x))))

time_series_longer_winner <- pivot_longer(time_series_winner, cols = c(1:120), values_to = "Winning_Portfolio_Return", names_to = "time")%>%
  mutate(time = str_extract(time, "\\d{4}\\.\\d{2}"),
         time = ym(time),
         month = month(time),
         year = year(time))%>%
  mutate(
    wealth_winner = 100 * cumprod(coalesce(Winning_Portfolio_Return, 1))    # $100 growing month by month
  )

returns_data <- left_join(time_series_longer_nasdaq, time_series_longer_winner, by = join_by(time, month, year))

write_csv(tableau, "tableau.csv")
write_csv(returns_data, "Returns Data.csv")
